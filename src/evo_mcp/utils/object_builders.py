...
class BaseObjectBuilder(ABC):
    ...
    def build_category_attribute(self, data: "pd.Series", name: str) -> "CategoryAttribute_V1_1_0":
        ...
        lookup_df = pd.DataFrame({"key": range(1, len(unique_values) + 1), "value": unique_values})
    ...
    def build_continuous_attribute(self, data: "pd.Series", name: str) -> "ContinuousAttribute_V1_1_0":
    ...
    def build_attributes(
        self, df: pd.DataFrame, columns: list[str], exclude_columns: Optional[list[str]] = None
    ) -> list:
    ...
    def build_bounding_box(self, df: pd.DataFrame, x_col: str, y_col: str, z_col: str) -> "BoundingBox_V1_0_1":
        ...
        for key, value in bbox_values.items():
            if not np.isfinite(value):
                raise ValueError(
                    f"Bounding box {key} is not finite: {value}. Check coordinate data for infinite values."
                )
...
class LineSegmentsBuilder(BaseObjectBuilder):
    ...
    def build(
        ...
        vertex_attrs = self.build_attributes(vertices_df, vertex_attribute_columns, exclude_columns=list(coord_cols))
        ...
        segment_attrs = self.build_attributes(segments_df, segment_attribute_columns, exclude_columns=list(index_cols))
...
class DownholeCollectionBuilder(BaseObjectBuilder):
    ...
    def build_hole_id_lookup(self, hole_ids: list[str]) -> pd.DataFrame:
        ...
        return pd.DataFrame({"key": range(1, len(hole_ids) + 1), "value": hole_ids})
    ...
    def save_path_array(self, df: pd.DataFrame, depth_col: str, azimuth_col: str, dip_col: str) -> dict:
        ...
        table = pa.Table.from_pandas(
            data.astype("float64"),
            schema=pa.schema([("distance", pa.float64()), ("azimuth", pa.float64()), ("dip", pa.float64())]),
            preserve_index=False,
        )
        return self.data_client.save_table(table=table)
    ...
    def build_hole_index_map(self, data_df: pd.DataFrame, id_column: str, hole_id_lookup: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in data_df.iterrows():
            ...
            if hole_id != current_hole:
                if current_hole is not None and current_hole in id_to_key:
                    records.append(
                        {"hole_index": id_to_key[current_hole], "offset": current_offset, "count": current_count}
                    )
        ...
        if current_hole is not None and current_hole in id_to_key:
            records.append({"hole_index": id_to_key[current_hole], "offset": current_offset, "count": current_count})
        ...
        for key in all_keys - existing_keys:
            records.append({"hole_index": key, "offset": 0, "count": 0})
    ...
    def save_holes_mapping(self, holes_df: pd.DataFrame) -> dict:
        ...
        table = pa.Table.from_pandas(
            holes_df,
            schema=pa.schema([("hole_index", pa.int32()), ("offset", pa.uint64()), ("count", pa.uint64())]),
            preserve_index=False,
        )
        return self.data_client.save_table(table=table)
    ...
    def build_location(
        ...
        holes = DownholeCollection_V1_3_0_Location_Holes.from_dict(self.save_holes_mapping(survey_holes_df))
    ...
    def build_interval_collection(
        ...
        intervals = Intervals_V1_0_1(start_and_end=FloatArray2_V1_0_1.from_dict(intervals_ref))

        # Holes mapping
        holes_df = self.build_hole_index_map(interval_df, id_col, hole_id_lookup)
        holes = DownholeCollection_V1_3_0_Location_Holes.from_dict(self.save_holes_mapping(holes_df))
    ...
    def build(
        ...
        if interval_collections:
            for coll_config in interval_collections:
                coll_df = (
                    coll_config["dataframe"]
                    .sort_values(by=[coll_config["id_col"], coll_config["from_col"]])
...
class DownholeIntervalsBuilder(BaseObjectBuilder):
    ...
    def build(
        ...
        start = Locations_V1_0_1(coordinates=FloatArray3_V1_0_1.from_dict(start_ref))

        # Build end locations
        end_ref = self.save_float_array3(df, end_x_column, end_y_column, end_z_column)
        end = Locations_V1_0_1(coordinates=FloatArray3_V1_0_1.from_dict(end_ref))

        # Build midpoint locations
        mid_ref = self.save_float_array3(df, mid_x_column, mid_y_column, mid_z_column)
        mid_points = Locations_V1_0_1(coordinates=FloatArray3_V1_0_1.from_dict(mid_ref))

        # Build from-to intervals
        intervals_ref = self.save_float_array2(df, from_column, to_column)
        from_to = FromTo_V1_0_1(intervals=Intervals_V1_0_1(start_and_end=FloatArray2_V1_0_1.from_dict(intervals_ref)))

        # Build hole ID category data
        clean_ids = df[hole_id_column].fillna("").astype(str)
        unique_ids = clean_ids.unique()
        lookup_df = pd.DataFrame({"key": range(1, len(unique_ids) + 1), "value": unique_ids})
        value_to_key = dict(zip(lookup_df["value"], lookup_df["key"]))
