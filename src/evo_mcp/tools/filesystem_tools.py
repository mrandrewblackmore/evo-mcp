...
def register_filesystem_tools(mcp):
    ...
    @mcp.tool()
    async def configure_local_data_directory(directory_path: str = "") -> dict:
    ...
    @mcp.tool()
    async def list_local_data_files(file_pattern: str = "*.csv", recursive: bool = True) -> dict:
        ...
        if not data_dir.exists():
            return {"error": f"Data directory does not exist: {data_dir}", "status": "directory_missing"}
    ...
    @mcp.tool()
    async def preview_csv_file(file_path: str, max_rows: int = 10) -> dict:
        ...
        if not file_path.exists():
            return {"error": f"File not found: {file_path}", "status": "file_missing"}
